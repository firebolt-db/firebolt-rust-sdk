...
async fn test_authenticate_success() {
    let config = TestConfig::from_env().expect("Failed to load test configuration from environment variables");
    ...
    match result {
        Ok((access_token, expiration_timestamp)) => {
            ...
            println!("   Expires in: {} seconds", expiration_timestamp - current_time);
        }
        Err(error) => {
